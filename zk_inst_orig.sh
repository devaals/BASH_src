#!/bin/bash
#Скрипт развёртывания ZooKeeper 3.4.6
#Авторы: Артём Верещага a.vereschaga@vniias.ru Владислав Патрушев v.patryshev@vniias.ru
...
server.1=10.64.6.18:3001:3002
server.2=10.64.6.19:3001:3002
server.3=10.64.6.24:3001:3002
server.4=10.64.6.25:3001:3002
server.5=10.64.6.30:3001:3002
server.6=10.64.6.31:3001:3002
EOF
	if [ "$server" == "10.64.6.18" ]; then echo '1' > $zoo/data/myid
		elif [ "$server" == "10.64.6.19" ]; then echo '2' > $zoo/data/myid
		elif [ "$server" == "10.64.6.24" ]; then echo '3' > $zoo/data/myid
		elif [ "$server" == "10.64.6.25" ]; then echo '4' > $zoo/data/myid
		elif [ "$server" == "10.64.6.30" ]; then echo '5' > $zoo/data/myid
		elif [ "$server" == "10.64.6.31" ]; then echo '6' > $zoo/data/myid
